...
- [ ] T009 Create shared DB utilities in src/aizk/db.py: get_engine() with SQLite PRAGMAs (WAL, synchronous=NORMAL, busy_timeout, foreign_keys), get_session(), create_db_and_tables()
...
- [ ] T022 [P] [US1] Unit tests for utilities in tests/conversion/unit/test_url_utils.py (normalize_url, detect_content_type, detect_source_type, extract_arxiv_id, extract_github_repo)
...
- [ ] T025 [P] Implement arXiv ID extraction in src/aizk/conversion/utilities/url_utils.py: extract_arxiv_id(url) using regex pattern
- [ ] T026 [P] Implement GitHub owner/repo extraction in src/aizk/conversion/utilities/url_utils.py: extract_github_repo(url) using regex pattern
- [ ] T027 Implement URL fetcher in src/aizk/conversion/workers/fetcher.py: fetch_url(url, timeout, max_size) with retry logic (3 attempts, exponential backoff), returns content bytes and content_type
- [ ] T028 Implement arXiv HTML/PDF fetcher in src/aizk/conversion/workers/fetcher.py: fetch_arxiv(arxiv_id) prioritizes export.arxiv.org/html, falls back to export.arxiv.org/pdf
- [ ] T029 [P] Implement GitHub README fetcher in src/aizk/conversion/workers/fetcher.py: fetch_github_readme(owner, repo) tries README.md, README.rst, README.txt, README in main/master branches
- [ ] T030 Implement Docling HTML pipeline in src/aizk/conversion/workers/converter.py: convert_html(html_bytes, temp_dir) returns markdown_text and list of figure paths
- [ ] T031 [P] Implement Docling PDF pipeline in src/aizk/conversion/workers/converter.py: convert_pdf(pdf_bytes, temp_dir) returns markdown_text and list of figure paths
- [ ] T032 Implement S3 upload in src/aizk/conversion/storage/s3_client.py: upload_file(local_path, s3_key) with verification (check ETag or HTTP 200)
- [ ] T033 [P] Implement S3 batch upload in src/aizk/conversion/storage/s3_client.py: upload_artifacts(temp_dir, s3_prefix) uploads markdown, figures, and manifest; returns list of uploaded keys
- [ ] T034 [P] Implement manifest generation in src/aizk/conversion/storage/manifest.py: generate_manifest(bookmark, job, artifacts) creates manifest.json dict with version, source, conversion, artifacts sections per data-model.md schema. **Manifest must store absolute S3 URIs** (s3://bucket/aizk_uuid/filename) for all artifact keys (markdown, figures) to ensure durability and portability. ConversionOutput.manifest_key must store full S3 URI or absolute path, not just prefix.
...
- [ ] T040 Implement POST /v1/jobs endpoint in src/aizk/conversion/api/routes/jobs.py: create or lookup bookmark, compute idempotency_key, check for duplicate, create ConversionJob with status=NEW→QUEUED, return 201 or 200 with existing job
