...
- [x] T027 Implement URL fetcher in src/aizk/conversion/workers/fetcher.py: fetch_url(url, timeout, max_size) with retry logic (3 attempts, exponential backoff), returns content bytes and content_type. Implement fetch_karakeep_pdf_asset(karakeep_id, asset_id) using karakeep_client to fetch PDF asset bytes when not provided in submission.
- [ ] T028 Implement arXiv content handler in src/aizk/conversion/workers/fetcher.py: fetch_arxiv_content(bookmark, pdf_asset_bytes=None) handles three cases: (1) if bookmark source URL is from abstract page (arxiv.org/abs), extract arxiv_id using aizk.utilities.url_utils and download PDF using aizk.utilities.arxiv.download_pdf(); (2) if bookmark has PDF asset, use provided pdf_asset_bytes or fetch from KaraKeep if None; (3) if bookmark has HTML content and arxiv_pdf_url in metadata, download PDF from arxiv_pdf_url. Returns PDF bytes for conversion.
