...
- [ ] T037a Implement upload retry handler in src/aizk/conversion/workers/worker.py: process_upload_pending_jobs() queries jobs with status=UPLOAD_PENDING ordered by last_error_at, retries upload while the temporary workspace exists, and falls back to full retry (FAILED_RETRYABLE) if artifacts are missing.
