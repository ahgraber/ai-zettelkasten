...
def test_upgrade_produces_schema_matching_create_all(tmp_path):
    ...
    ``run_migrations`` imports every stage's models (conversion, graph,
    pipeline) via ``aizk.db.migrations.env``, so by the time the baseline
    ``create_all`` call below runs, ``SQLModel.metadata`` already carries the
    full cross-stage table set — this is the one parity check that spans all
    tables, including ``pipeline_runs``/``pipeline_events`` and the mention
    tables, not just conversion's own.
    """
    ...
    for table in baseline_tables:
        assert_table_schema_equivalent(migrated_inspector, baseline_inspector, table)
