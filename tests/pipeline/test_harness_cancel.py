...
def test_running_cancelled_promptly() -> None:
    ...
    the harness to cancel it; the adapter's cancel hook releases the worker, and
    the harness finalizes the unit as cancelled within a bounded interval.
    """
    ...
    unit_id = repo.enqueue("running", behavior=_block_until_cancelled)
    ...
    with no_thread_leaks(action="raise"):
        ...
        harness.cancel_handle(unit_id)
        driver.join(timeout=10.0)
        elapsed = time.monotonic() - cancel_observed
        assert not driver.is_alive(), "harness drained after cancellation"

    assert elapsed < 5.0, f"cancellation took {elapsed:.2f}s — not within a bounded interval"
    assert str(unit_id) in repo.recorded.cancelled, "cancel hook was invoked"
    assert repo.get_status(unit_id) == WorkUnitStatus.CANCELLED.value, "unit reached the cancelled outcome"


def test_queued_cancel_skipped() -> None:
    """A unit cancelled while queued is skipped and never executed.

    A unit seeded in a cancelled state is not eligible, so the harness drains
    without ever calling ``execute`` for it.
    """
    engine = create_stub_engine()
    repo = StubStageRepository(engine, concurrency_limit=2)

    cancelled_id = repo.enqueue("cancelled-while-queued", status=WorkUnitStatus.CANCELLED.value)
    runnable_id = repo.enqueue("runnable")
    ...
    assert str(cancelled_id) not in repo.recorded.execute_started, "queued-cancelled unit was never executed"
    assert repo.get_status(cancelled_id) == WorkUnitStatus.CANCELLED.value, "it stays cancelled"
    assert repo.get_status(runnable_id) == WorkUnitStatus.SUCCEEDED.value, "the runnable unit still ran"
