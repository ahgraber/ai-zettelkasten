...
from aizk.conversion.workers import errors as errors_mod, loop, orchestrator, uploader
...
def test_upload_converted_shortcut_manifest_upload_is_retryable(
    ...
    The worker's upload retry policy wraps ``_execute_upload`` (orchestrator.py),
    not ``_prepare_upload``. The shortcut path therefore must not perform any S3
