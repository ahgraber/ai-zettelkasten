"""Integration tests for the conversion orchestrator's KEPT helpers.

The per-job orchestration loop (and its
cancellation / timeout / upload-retry / skip-spawn branches) was retired with
the conversion engine; that behavior now lives in the runner adapter's
``ConversionStageHandler.execute`` and is
covered by ``tests/conversion/unit/test_handler.py`` (execute: upload retry +
exhaustion, timeout rebind, cancelled, reported-child, subprocess-error,
content-hash marker, deadline; finalize: terminal writes + egress scrub) and the
real-subprocess proofs in ``test_worker_lifecycle.py`` /
``test_handler_runner_lifecycle.py`` / ``test_worker_e2e.py``.

What remains here exercises the orchestrator helpers that survived the cutover
because the adapter reuses them: the subprocess entrypoint's process-group
setup, the cancellation preflight, the uploader's content-hash dedup shortcut,
and best-effort Source enrichment.
"""
...
from aizk.conversion.workers import errors as errors_mod, orchestrator, uploader
...
def test_process_group_creation_called_in_subprocess(monkeypatch) -> None:
    ...
    monkeypatch.setattr(orchestrator, "load_process_dotenv_once", _short_circuit)
...
# ---------------------------------------------------------------------------
# Uploader: content-hash dedup shortcut
# ---------------------------------------------------------------------------
