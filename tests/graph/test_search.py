...
def test_search_returns_active_chunk_not_superseded(session: Session, provider: Fts5SearchProvider) -> None:
    ...
    _persist_chunks(session, [active, extra], markdown_hash=_HASH_B)
    ...
    assert [r.chunk_id for r in active_hits] == [active.chunk_id]
...
def test_search_drops_contextualized_hit_when_chunk_left_active_manifest(
    ...
    run_c1 = _persist_chunks(session, [chunk_c1], markdown_hash=_HASH_A)
    _contextualize(
        session,
        [chunk_c1],
    ...
    _persist_chunks(session, [chunk_c2], markdown_hash=_HASH_B)
    ...
    assert [r.chunk_id for r in provider.search("antidisestablishment", SearchKind.CHUNK)] == [chunk_c2.chunk_id]
...
def test_search_type_filter_contextualized_only_term(session: Session, provider: Fts5SearchProvider) -> None:
    ...
    run_id = _persist_chunks(session, [chunk])
    _contextualize(
        session,
        [chunk],
    ...
    assert [r.chunk_id for r in under_ctx] == [chunk.chunk_id]
    assert under_ctx[0].matched_in_contextualized is True
    assert under_ctx[0].matched_in_chunk is False
    assert [r.chunk_id for r in under_either] == [chunk.chunk_id]
    assert under_chunk == []
...
def test_search_type_filter_raw_only_term(session: Session, provider: Fts5SearchProvider) -> None:
    ...
    assert [r.chunk_id for r in under_chunk] == [chunk.chunk_id]
...
def test_search_type_filter_self_contained_matches_contextualized_by_raw(
    ...
    run_id = _persist_chunks(session, [chunk])
    _contextualize(session, [chunk], [""], chunking_run_id=run_id)  # empty => self-contained
...
def test_search_orders_by_document_relevance_then_span_start(
    ...
    assert [r.chunk_id for r in results] == [a_early.chunk_id, a_late.chunk_id, b_chunk.chunk_id], (
...
def test_search_finds_literal_special_character_term(session: Session, provider: Fts5SearchProvider) -> None:
    ...
    assert [r.chunk_id for r in results] == [chunk.chunk_id]
...
def test_search_excludes_source_without_active_variant_run(
    ...
    assert [r.chunk_id for r in provider.search("uncontextualized", SearchKind.CHUNK)] == [chunk.chunk_id]
    ...
    assert [r.chunk_id for r in either] == [chunk.chunk_id]
