...
def _seed_chunks(
    ...
    *,
    ...
    """Persist a chunk set via the real persist path.

    Returns its chunking run id and the persisted chunks, each carrying the
    surrogate ``chunk_id`` assigned at persistence (in document order).
    """
...
def test_explorer_spine_lists_chunks_in_span_start_order_with_facts(
    ...
    run_id, persisted = _seed_chunks(db_session, [late, early], source_id=source_id, conversion_output_id=output_id)
    late_p, early_p = persisted
    ...
    response = explorer_client.get("/ui/explore/chunks", params={"source_id": source_id})

    assert response.status_code == 200
    body = response.text
    # Reading order: the early chunk (span_start 10) precedes the late one (span_start 900).
    assert body.index(early_p.chunk_id) < body.index(late_p.chunk_id)
    ...
    assert f"{early_p.char_count} chars" in body
...
def test_explorer_detail_shows_revision_distinct_from_raw_with_provenance(
    ...
    run_id, persisted = _seed_chunks(db_session, [chunk], source_id=source_id, conversion_output_id=output_id)
    chunk_p = persisted[0]
    revision = "scaled dot-product attention improves throughput"
    _seed_contextualization(
        db_session, [chunk_p], [revision], source_id=source_id, conversion_output_id=output_id, chunking_run_id=run_id
    )

    response = explorer_client.get("/ui/explore/chunks", params={"source_id": source_id, "chunk_id": chunk_p.chunk_id})
...
def test_explorer_detail_self_contained_shows_raw_marked_with_same_provenance(
    ...
    chunk_p = persisted[0]
    _seed_contextualization(
        db_session, [chunk_p], [""], source_id=source_id, conversion_output_id=output_id, chunking_run_id=run_id
    )
...
def test_explorer_select_result_opens_document_at_chunk_with_contextualized_detail(
    ...
    # The search row exposes the select target as an hx-get to /ui/explore/chunks.
    search = explorer_client.post(
        "/ui/explore/chunks/search", data={"query": "scaled dot-product attention", "kind": "either"}
    )
    assert search.status_code == 200
    assert "/ui/explore/chunks?source_id=" in search.text
    assert chunk_p.chunk_id in search.text
    ...
    opened = explorer_client.get(
        "/ui/explore/chunks",
        params={"source_id": source_id, "chunk_id": chunk_p.chunk_id},
...
def test_explorer_search_highlights_contextualized_only_and_both_sides(
    ...
    run_id, persisted = _seed_chunks(db_session, [ctx_only, both], source_id=source_id, conversion_output_id=output_id)
    ctx_only_p, both_p = persisted
    _seed_contextualization(
        db_session,
        [ctx_only_p, both_p],
        [
    ...
    response = explorer_client.post("/ui/explore/chunks/search", data={"query": "photosynthesis", "kind": "either"})
    ...
    assert f'data-chunk-id="{ctx_only_p.chunk_id}"' in body
    assert "match: contextualized only" in body
    # The both-sides chunk is a single row marked on both.
    assert f'data-chunk-id="{both_p.chunk_id}"' in body
...
def test_explorer_search_contextualized_only_not_marked_in_raw(
    ...
    response = explorer_client.post("/ui/explore/chunks/search", data={"query": "scaled", "kind": "either"})
...
def test_explorer_search_empty_query_renders_empty_partial(
    ...
    _run_id, persisted = _seed_chunks(db_session, [chunk], source_id=source_id, conversion_output_id=output_id)
    chunk_p = persisted[0]

    empty = explorer_client.post("/ui/explore/chunks/search", data={"query": "", "kind": "either"})
    whitespace = explorer_client.post("/ui/explore/chunks/search", data={"query": "   \t  ", "kind": "either"})
    ...
    assert chunk_p.chunk_id not in empty.text
    assert whitespace.status_code == 200
    assert 'class="result-row"' not in whitespace.text
    assert chunk_p.chunk_id not in whitespace.text
...
def test_explorer_search_syntax_characters_do_not_error(
    ...
    literal = explorer_client.post("/ui/explore/chunks/search", data={"query": 'wildcard* "quoted"', "kind": "either"})
    boolean_word = explorer_client.post("/ui/explore/chunks/search", data={"query": "AND OR NEAR", "kind": "either"})

    # Special-char query never errors the page and returns the literal match.
    assert literal.status_code == 200
    assert f'data-chunk-id="{chunk_p.chunk_id}"' in literal.text
...
def test_explorer_source_mid_contextualization_shows_raw_no_representation_no_memo(
    ...
    spine = explorer_client.get("/ui/explore/chunks", params={"source_id": source_id})
    assert spine.status_code == 200
    assert chunk_p.chunk_id in spine.text
    ...
    detail = explorer_client.get("/ui/explore/chunks", params={"source_id": source_id, "chunk_id": chunk_p.chunk_id})
    ...
    raw_hit = explorer_client.post("/ui/explore/chunks/search", data={"query": "uncontextualized", "kind": "either"})
    assert raw_hit.status_code == 200
    assert chunk_p.chunk_id in raw_hit.text
    memo_hit = explorer_client.post(
        "/ui/explore/chunks/search", data={"query": "floccinaucinihilipilification", "kind": "either"}
    )
