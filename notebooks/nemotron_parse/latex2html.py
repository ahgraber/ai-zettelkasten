...
def parse_braced_argument(text, i):
    """Given text and an index i that should point at an opening '{', return a tuple (argument_content, new_index) where argument_content is the full string inside the balanced braces and new_index is the position just after the matching '}'."""
    if i >= len(text) or text[i] != "{":
        raise ValueError("Expected '{{' at position {}".format(i))  # {{ escapes the brace
...
def parse_command(text, i):
    r"""Parse a \multirow or \multicolumn command starting at index i.
    ...
    for arg_index in range(3):
        if i >= len(text) or text[i] != "{":
            raise ValueError(  # {{ escapes the brace
                "Expected '{{' for argument {} at position {}".format(arg_index + 1, i)
            )
...
def clean_multi_cells(text):
    r"""Process an arbitrary LaTeX text string and look for occurrences of \multirow or \multicolumn commands.
    ...
    while i < len(text):
        ...
        if idx_multir == -1:
            ...
            next_idx = min(idx_multir, idx_multic)
...
def parse_brace(s, pos):
    """Given a string s and an index pos pointing to an opening '{', returns a tuple (content, new_pos) where content is the string between the matching braces (handling nested braces) and new_pos is the index just after the closing '}'."""
...
def latex_table_to_html(latex_str, add_head_body=False):  # noqa: C901
    ...
    def convert_table(match):  # noqa: C901
