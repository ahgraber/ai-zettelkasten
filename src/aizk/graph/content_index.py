...
#: column; ``kind``/``chunk_id``/``run_id``/``doc_id`` are ``UNINDEXED`` so the
...
#: representation is its raw text); ``doc_id`` and the raw text come from the joined
...
_INSERT_CHUNK_SQL = (
    "INSERT INTO graph_content_fts (text, kind, chunk_id, run_id, doc_id) "
    "VALUES (:text, 'chunk', :chunk_id, :run_id, :doc_id)"
)

#: Insert one contextualized row into the index.
_INSERT_CONTEXTUALIZED_SQL = (
    "INSERT INTO graph_content_fts (text, kind, chunk_id, run_id, doc_id) "
    "VALUES (:text, 'contextualized', :chunk_id, :run_id, :doc_id)"
)
...
def index_chunk_content(
    ...
    *,
    ...
    Args:
        ...
        doc_id: The source identity (``str(aizk_uuid)``).
    """
    session.connection().execute(
        text(_INSERT_CHUNK_SQL),
        {"text": text_, "chunk_id": chunk_id, "run_id": run_id, "doc_id": doc_id},
    )
