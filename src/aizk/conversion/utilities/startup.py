...
def probe_karakeep(config: ConversionConfig) -> None:
    """Verify KaraKeep API is reachable and credentials are valid.

    Reads ``base_url`` and ``api_key`` from ``config.fetcher.karakeep``
    (populated via ``AIZK_FETCHER__KARAKEEP__*`` env vars).

    Raises:
        StartupValidationError: If the KaraKeep API is unreachable, returns
            an error, or required env vars are missing.
    """
    base_url = config.fetcher.karakeep.base_url
    api_key = config.fetcher.karakeep.api_key
...
def log_feature_summary(config: ConversionConfig, role: str) -> None:
    ...
    elif not config.converter.docling.picture_classification_enabled:
        features["picture_classification"] = {
            "status": "disabled",
