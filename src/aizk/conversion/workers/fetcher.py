...
# ---------------------------------------------------------------------------
# Lazy re-exports for Stage 4 fetcher adapters
# ---------------------------------------------------------------------------
# Use __getattr__ to avoid circular imports: the adapter modules import
# fetch_karakeep_asset / fetch_arxiv_pdf from *this* module, so a top-level
# import of the adapter classes here would create an import cycle.

_ADAPTER_EXPORTS: dict[str, tuple[str, str]] = {
    "KarakeepBookmarkResolver": (
        "aizk.conversion.adapters.fetchers.karakeep",
        "KarakeepBookmarkResolver",
    ),
    "ArxivFetcher": (
        "aizk.conversion.adapters.fetchers.arxiv",
        "ArxivFetcher",
    ),
    "GithubReadmeFetcher": (
        "aizk.conversion.adapters.fetchers.github",
        "GithubReadmeFetcher",
    ),
    "UrlFetcher": (
        "aizk.conversion.adapters.fetchers.url",
        "UrlFetcher",
    ),
    "InlineContentFetcher": (
        "aizk.conversion.adapters.fetchers.inline",
        "InlineContentFetcher",
    ),
    "SingleFileFetcher": (
        "aizk.conversion.adapters.fetchers.singlefile",
        "SingleFileFetcher",
    ),
}
...
def __getattr__(name: str) -> object:  # pragma: no cover — thin shim
    if name in _ADAPTER_EXPORTS:
        module_path, attr = _ADAPTER_EXPORTS[name]
        import importlib
