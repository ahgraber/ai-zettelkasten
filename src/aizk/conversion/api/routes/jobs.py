...
def submit_job(
    ...
    converter_name = request.app.state.converter_name
    config_snap = request.app.state.converter_config_snapshot
    ...
    session.add(job)
    # Flush so job.id is assigned before the origin event captures it.
    session.flush()
    record_transition(
        session,
        job,
