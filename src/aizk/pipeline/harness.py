"""The pipeline-stage harness: the current embedded orchestration engine.

The harness is the faithful generic extraction of ``aizk.conversion.workers``
(loop / orchestrator / shutdown / supervision). It drives any stage that
supplies a :class:`~aizk.pipeline.repository.StageRepository` over its own store
and owns the engine-side concerns:

- the claim/lease loop and bounded concurrency over a thread pool;
- eligibility + submission ordering (delegated to the repository's claim query,
...
- retry-wait gating (the repository's eligibility query; the harness simply
  re-polls);
...
Transaction discipline: the harness owns the DB session and the
...
harness's bookkeeping; the repository never commits.
"""
...
from aizk.pipeline.repository import StageRepository, WorkUnitHandle
...
class StageMetrics(Protocol):
    """Operational-metrics sink the harness emits lifecycle counters through.

    A minimal counter/gauge interface so the harness stays decoupled from any
...
class StageHarness(Generic[WorkUnitHandle]):
    """Drives a single :class:`StageRepository` through the generic lifecycle.

    One harness instance owns one stage's loop. Two stages share a process by
    constructing two harnesses (each with its own repository, engine, and
    :class:`ShutdownController`); no harness state is module-global.
    ...
    def __init__(
        self,
        repository: StageRepository[WorkUnitHandle],
        engine: "Engine",
        *,
        ...
        """Construct a harness over ``repository`` backed by ``engine``.

        Args:
            repository: The stage adapter the harness drives.
            engine: SQLite engine the harness opens ``BEGIN IMMEDIATE``
            ...
            cancel_grace: Seconds to wait — only on the drain-timeout survivor
                path — for a cancelled unit to finish before the harness gives up
    ...
    def metrics(self) -> StageMetrics:
        """The operational-metrics sink this harness emits through."""
    ...
    def _begin_immediate(self) -> "Iterator[Session]":
        ...
        commit" convention: the repository's transition writes are staged inside
        this block and committed here, co-committed with the harness bookkeeping.
        """
    ...
    def validate_dependencies(self) -> None:
        """Run the repository's startup dependency validation, gating acceptance.

        Raises whatever the repository raises. Until this completes
        ...
        self._repo.validate_dependencies()
    ...
    def set_process_title(self) -> None:
        """Advertise this harness's stage role in the process title for operators.

        Multi-harness semantics: the process title is a single process-global
        slot, so ``setproctitle`` is last-write-wins in a shared process. When
        two harnesses share a process, the most recently set title is the one
        operators see; the title is an advisory role hint, not a per-harness
        identifier.
        """
        try:
    ...
    def recover_stale(self) -> int:
        ...
        recording each recovery's cause in its transition event (the repository
    ...
    def _recover_stale(self) -> int:
        ...
        try:
            with self._begin_immediate() as session:
                recovered = self._repo.recover_stale(session)
    ...
    def _claim_next(self) -> WorkUnitHandle | None:
        ...
        gating) lives in the repository; the harness only drives it. The spec's
        "begin in submission order" is this claim/selection order — units are
        dispatched oldest-first; the harness does not serialize the worker
        threads' actual start instants (they run concurrently up to the limit).
        """
        try:
            with self._begin_immediate() as session:
                handle = self._repo.claim_next(session)
        ...
        if handle is not None:
            ...
            # Source-identity / run correlation (aizk_uuid, run_id) lives on the
            # repository's pipeline_events rows, not the harness (narrow seam);
            # the harness logs stage + work_unit_ref.
            logger.info(
                "Claimed work-unit",
    ...
    def _run_unit(self, handle: WorkUnitHandle, registered: threading.Event) -> Any:
        ...
        Waits until the harness has registered this unit's concurrency slot
        ...
        terminal outcome. Keeps no harness state — pure call into the adapter.
        """
        registered.wait()
        return self._repo.execute(handle)
    ...
    def _submit(self, executor: ThreadPoolExecutor, handle: WorkUnitHandle) -> None:
        ...
        ``max_workers`` (the repository's ``concurrency_limit``) bounds
        concurrency; the gate is purely for slot-visibility, not bounding.
        """
        now = self._clock()
        timeout_seconds = self._repo.timeout.total_seconds()
    ...
    def _finalize_slot(self, future: Future, slot: _Slot[WorkUnitHandle]) -> None:
        ...
        The slot is the harness's local record that this unit ran; it is dropped
        ...
        unit stays known to the harness so the next :meth:`_reap` pass re-attempts
        ...
        the harness forgot it, inviting delayed stale-recovery and duplicate
        execution.
        """
        handle = slot.handle
        ...
        try:
            with self._begin_immediate() as session:
                self._repo.finalize(session, handle, outcome)
        except (OperationalError, DBAPIError):
            logger.exception(
    ...
    def _resolve_outcome(self, future: Future, slot: _Slot[WorkUnitHandle]) -> TerminalOutcome:
        ...
        Timeout and cancellation are harness-determined and take precedence over
        ...
        # The harness never calls ``future.cancel()``, so ``result()`` below cannot
        # raise ``CancelledError`` on this finished, non-timed-out, non-cancelled future.
        exc = future.exception()
        if exc is not None:
            return self._repo.map_result(exc)
        return self._repo.map_result(future.result())
    ...
    def run_until_idle(self, *, max_iterations: int = 100_000) -> None:
        ...
        with ThreadPoolExecutor(max_workers=self._repo.concurrency_limit) as executor:
            for _ in range(max_iterations):
                self._enforce_deadlines()
                self._reap()
                claimed = False
                if not self._shutdown.is_shutdown_requested():
                    while len(self._slots) < self._repo.concurrency_limit:
    ...
    def run(self) -> int:
        ...
        self._ensure_validated()
        logger.info(
            "Starting stage harness loop",
            extra={
                "stage": self._stage,
                "concurrency_limit": self._repo.concurrency_limit,
                "drain_timeout": self._drain_timeout,
            },
        )
        force_terminated = False
        executor = ThreadPoolExecutor(max_workers=self._repo.concurrency_limit)
        try:
    ...
    def _ensure_validated(self) -> None:
        ...
        Enforces the startup gate: if the repository's validation raises, the
...
__all__ = ["InMemoryMetrics", "StageHarness", "StageMetrics"]
