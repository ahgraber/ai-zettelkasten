...
- **Run/dataset-version primitive**: runs are recorded on the shared `aizk.pipeline.run` (`PipelineRun`, `record_run`) and lifecycle transitions on `aizk.pipeline.events`; all runs are scoped by the durable source identity `str(aizk_uuid)`.
