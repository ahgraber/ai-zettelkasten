# Mention-store Specification

> Synced from change `mention-extraction-foundation` on 2026-07-16

## Purpose

The mention-store capability is the append-only lexical ground-truth dataset that entity canonicalization is later calibrated against.
Each mention records its verbatim surface form and complete provenance — source chunk, anchor class (`source` or `revision`) and span coordinates, the input text and span read, and the extraction run that produced it — but no context embedding, which is recomputed on demand at canonicalization time.
Mentions are produced by per-source extraction runs on the shared run primitive: one active run per source, a surrogate mention identity unique per anchor class within a run, and a stable `source_occurrence_key` for cross-run continuity.
Records are immutable; re-extraction opens a superseding run and never mutates or duplicates prior rows, and co-occurrence is resolvable through a link table rather than stored on the mention row.
