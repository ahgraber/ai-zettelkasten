...
- [ ] Contextualization pending query, generalizing `latest_output_ids_per_source` with an anti-join on work-units.
...
- [ ] Extraction pending query, generalizing `_sources_with_active_chunking_run` with an anti-join on work-units.
...
- [ ] State-only evidence for both queries: evaluating twice against identical state yields the identical set.

## 4. Staleness derivation (extraction)

- [ ] Staleness query: active mention-extraction run's recorded `upstream_derivation_key` vs. the key `_resolve_upstream_derivation_key` yields for current state.
...
- [ ] Resolver-conformance test pinning the staleness query and the execute-time resolver to the same verdict on the same state.
- [ ] Explicit stale-is-not-pending test: a stale source does not appear in the extraction pending set.

## 5. Admission loop in the workers

- [ ] Per-stage admission adapter (pending query + enqueue + capacity + enable flag) for both graph stages, with a queryable-declaration test (declaring stage reports it; absent adapter reports none).
- [ ] Admission enable/interval config fields (`admission_contextualization_enabled`, `admission_extraction_enabled`, `admission_interval_seconds`; default off), with a hermetic settings test.
- [ ] Admission loop in the contextualization worker; wiring covered in `tests/graph/test_worker_build.py`.
- [ ] Admission loop in the extraction worker; wiring covered in `tests/graph/test_extraction_worker_build.py`.
- [ ] Behavior tests (new `tests/graph/test_admission.py`):
...
- [ ] Loop lifecycle test: worker shutdown drains the admission loop cleanly, wrapped in the appropriate pyleak guard (`no_thread_leaks` / `no_task_leaks`).

## 6. Intake routes on the graph service

- [ ] Request/response schemas for intake, mirroring the conversion API's queue-full response shape.
- [ ] `POST /v1/contextualizations` resolving a conversion-output reference to the domain enqueue.
...
- [ ] `POST /v1/extractions` resolving a source identity to the domain enqueue.
  Tests in `tests/graph/test_extraction_operator_api.py`: the same matrix with 404 on unknown source.
- [ ] Equivalence tests, one per stage: an intake-created unit is identical to a domain-enqueued unit for the same work.

## 7. Extraction re-admission

- [ ] Requeue action in `job_actions.py`: eligible when terminal and source stale; transitions to `QUEUED` with attempts cleared and a requeue event co-committed.
  Tests: transition and event recorded; non-stale terminal unit skipped unaltered; non-terminal unit skipped unaltered.
- [ ] Declare the `Re-extract` action on the extraction stage's console descriptor.
  Console test: bulk application over a mixed selection applies to eligible units and reports the rest skipped.
- [ ] End-to-end test with the stub extractor: a re-admitted stale source re-executes reading current active inputs and the new run supersedes the prior one.
- [ ] Update the `extraction_workunit` module docstring: the re-trigger deferral is decided — describe the requeue mechanism that now exists.

## 8. Console visibility

- [ ] Optional `pending_count` / `pending_list` / `stale_count` callables on `StageDescriptor`, feature-detected like `failed_split`; descriptor tests in `tests/console/test_descriptors.py`.
...
- [ ] Pending-source listing on the stage's monitor page, using the monitor's title contract.
  Tests: listed set matches the derivation; listed count equals the dashboard count; non-declaring stage offers no listing.
- [ ] Dashboard stale count and monitor stale marking for extraction.
...
- [ ] Update `src/aizk/graph/README.md` (admission, intake, capacity, re-admission) and the conversion README's downstream note if its hand-off description is now stale.
- [ ] Capture `schemas/after/` snapshots and check the diff against `schemas/expected.md`.
- [ ] Full validation: `uv run pytest -n auto -m "not integration_lifecycle" tests/`, then `uv run pytest -m integration_lifecycle tests/`, then the pre-commit hooks over everything changed since `HEAD`.
