...
Enrichment SHALL follow last-writer-wins semantics across concurrent jobs for the same Source: the mutable columns are an advisory cache for UI/search, and each job's authoritative values are preserved in its own manifest. (Previously: the orchestrator derived these fields from KaraKeep bookmark structure for every job.
...
- Carry a typed `provenance` block describing the **terminal fetch state** — the identifiers the fetcher chain ultimately resolved to — keyed on the terminal ref's kind (e.g., `bookmark_id` for `karakeep_bookmark`, `arxiv_id` for `arxiv`, `owner`/`repo` for `github_readme`, `url` for `url`, `content_hash` for `inline_html`).
- Carry an optional `ingress` block that records the submitter-supplied `SourceRef` **only when it differs from the terminal provenance** (e.g., KaraKeep bookmark resolved to arXiv).
  When ingress and terminal ref are the same kind with the same identifiers, the `ingress` block SHALL be omitted.
- Move `karakeep_id` out of the top-level source block; it appears in either `provenance` (terminal `karakeep_bookmark` job) or `ingress` (when a KaraKeep bookmark resolved to a non-KaraKeep terminal ref).
...
#### Scenario: KaraKeep-terminal job records provenance without ingress
...
- **THEN** `version == "2.0"`, `provenance.kind == "karakeep_bookmark"` with the `bookmark_id` populated, and no `ingress` block is present

#### Scenario: KaraKeep-to-arxiv job records ingress alongside provenance
...
- **WHEN** the manifest is written
- **THEN** `provenance.kind == "arxiv"` with the `arxiv_id` populated, and `ingress.kind == "karakeep_bookmark"` with the original `bookmark_id` preserved

#### Scenario: Non-KaraKeep manifest serializes with nullable source fields

- **GIVEN** a conversion completes for a `UrlRef`-sourced job where the fetcher populated `url` but no `title`
- **WHEN** the manifest is written
- **THEN** `ManifestSource.title` is `null`, the manifest is valid, `provenance.kind == "url"`, and no `ingress` block is present
