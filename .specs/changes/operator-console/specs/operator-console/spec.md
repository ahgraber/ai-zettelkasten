...
graph-jobs-ui → operator-console

Reason: the capability generalizes from the contextualization jobs page into the single operator console covering every registered pipeline stage.
...
Serves: single-pane-operations, uniform-task-repair
...
> Previously: scoped to contextualization jobs only; conversion carried its own copy of this contract in `conversion-ui`.
...
> Previously: scoped to contextualization jobs with graph-stage eligibility; conversion carried its own copy in `conversion-ui`.
...
> Previously: defined only for contextualization units (the three graph-stage runs plus the event trail).
...
Removed because: superseded by the added "Console routes enforce the operator API perimeter", which carries the same contract generalized to every console surface, including the conversion stage's monitor.

### Requirement: Display the contextualization jobs table

Removed because: superseded by the added "Display the task monitor for every registered stage", which carries the same column and title contract generalized across all registered stages.
