...
### Decision: ExtractionRunModel

**Chosen:** Mentions are produced by **extraction runs** on the shared stage-run/dataset-version primitive (`pipeline_runs`), **one run per source** — `scope_id = str(source_id)`, exactly like the chunking and contextualization runs upstream.
No stage-specific run table exists: the primitive's partial unique index enforces at most one active run per source, `record_run`/`reuse_or_record_run` give atomic per-source supersession, and the run's `version_stamps` record `extractor_version`, `materializer_version`, and `input_policy`.
The run's `derivation_key` is a deterministic canonical-JSON key over semantic inputs only — `(extractor_version, materializer_version, input_policy, the consumed upstream run's derivation_key)`: the source's active contextualization run's key when `input_policy = contextualized`, else its active chunking run's key.
This matches the chunking stage's derivation-key precedent and satisfies `pipeline-identity`'s requirement that a derivation key be a fingerprint of semantic inputs and upstream derivation keys, embedding no database-local identifier.
...
Invalidation is run-level and lazy, per source: a changed extractor, materializer, input policy, or consumed upstream input changes that source's derivation key, so the source's next extraction opens a superseding run; other sources' active runs are untouched, and mention rows are immutable.
The invariant is **one active extraction run per source**; the corpus mention dataset is the union of the sources' active runs — the same way the active contextualized corpus is already the union of per-source contextualization runs.
...
The run records two distinct versions: **`extractor_version`** versions the NER extractor (model + config — spaCy or GLiNER2), and **`materializer_version`** versions the deterministic post-NER materialization logic that turns NER output into persisted mention records — span mapping (input→raw-chunk anchor), co-occurrence linking, and `mention_id`/`source_occurrence_key` derivation.
A change to that logic bumps `materializer_version` without changing the NER model, and vice versa; `input_policy` records the raw-vs-contextualized input toggle the run was produced under.

**Rationale:** A persisted mention is the output of a specific extraction run, not a timeless occurrence observed many ways — so modeling it as a run product (rather than an observation of an occurrence) keeps every downstream consumer reading active runs and ignoring superseded ones, without reasoning about per-observation history.
...
Classification enumerates rather than assigns: for each detected surface form, the materializer searches the raw chunk text for occurrences of that surface form.
...
Detections are never mapped to occurrences positionally or fuzzily; the classification is deterministic in the raw chunk text and the surface form alone, and any change to the rule is a `materializer_version` bump.
...
A revision-anchored mention is still a verifiable claim: its `input_ref` resolves to a persisted, immutable variant whose own provenance reaches the summary and raw chunk, and the extraction run's derivation key embeds the contextualization run's key, so an active mention's variant is always the active variant — never dangling.
...
Concrete pins: spaCy with the `en_core_web_sm` 3.8.0 pipeline installed as a direct URL-wheel dependency; `gliner2[local]==1.3.2` with the `fastino/gliner2-base-v1` weights pinned to an exact HuggingFace revision and pre-fetched to a local directory by an explicit setup step — runtime loads only from that path and never downloads.
Torch resolves from the PyTorch CPU wheel index on non-macOS platforms.
Both extractors live in an opt-in `ner` dependency group with lazy imports, so the contract suite (stub extractor) carries no model dependencies.
Extractor determinism (same input → identical detections at pinned versions) is verified by an explicit gate before retry idempotency is trusted; spaCy is deterministic by design, GLiNER2 is verified empirically.
...
### Decision: NoStoredBlockingKeys

**Chosen:** Mention rows carry **no blocking keys**.
Candidate-generation keys are a deterministic function of the persisted `surface_form`, so the entity-canonicalization change derives them at its own boundary and owns their index — including the selection of key families (normalized / phonetic / acronym / shingle / MinHash), which its create-vs-assign benchmarking evaluates against the real dataset.

**Rationale:** The mention store is lexical ground truth; a candidate-generation index is a consumer-side read model.
Storing keys here would denormalize a downstream consumer's concern onto immutable rows with no consumer until that change exists, and would pre-commit a key recipe before the benchmark that is supposed to choose it.
Nothing is lost by deferring: every recipe is recomputable from stored surface forms without touching this dataset.

**Alternatives considered:**

- **A minimal dependency-free recipe now (normalized / acronym / shingle).**
  An index without a consumer; still pre-commits a recipe ahead of the benchmark, and a later family change either bumps the materializer (re-extracting the corpus) or splits key ownership across two changes.
- **The full five-family recipe now.**
  Additionally forces the phonetic-library choice today, where the ecosystem has no maintained Double Metaphone implementation — an availability-driven pick instead of an evidence-driven one.
...
The extraction run is itself source-scoped (`scope_id = str(source_id)`), and the work unit's transition events carry `source_id`, so a source's progress is resolvable across stages (the runtime's cross-stage event requirement).
...
persisted chunk ──┬── contextualized variant available? ──► read variant   (input_kind=contextualized)
   ...
   (surrogate mention_id; anchor_kind        (flat: run_id, lo, hi, chunk_id;
    ...
    input_kind/ref; NO embedding)
                    │
                    ▼
   per-source extraction run on pipeline_runs (scope_id = str(source_id);
   derivation_key = canonical JSON(extractor/materializer versions, input_policy,
...
- **Version-mixed corpus view during an extractor/materializer rollout.**
  Per-source runs supersede lazily, so mid-backfill the union view mixes sources extracted under old and new versions.
